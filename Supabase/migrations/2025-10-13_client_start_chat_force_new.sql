...
-- Allow clients to force creation of a fresh chat conversation
DROP FUNCTION IF EXISTS public.client_start_chat_conversation(uuid);

create or replace function public.client_start_chat_conversation(
...
declare
  ...
  v_force_new boolean := coalesce(p_force_new, false);
begin
  ...
  if p_staff_user_id is not null then
    select coalesce(ut.type_name, p.user_type) as user_type, p.status
...
revoke all on function public.client_start_chat_conversation(uuid, boolean) from public;
grant execute on function public.client_start_chat_conversation(uuid, boolean) to authenticated;
