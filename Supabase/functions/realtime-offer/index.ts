...
serve(async (req) => {
  ...
  try {
    ...
    if (!sdp || typeof sdp !== "string") {
      return new Response(JSON.stringify({ error: "Missing SDP offer" }), { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } });
    }
    ...
    const { data: settings } = await supabase
      ...
      .select("enabled,instructions,voice,speech_rate")
      .eq("id", "GLOBAL")
      .maybeSingle();
    if (settings && settings.enabled === false) {
      return new Response(JSON.stringify({ error: "assistant_disabled" }), { status: 503, headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    const OPENAI_API_KEY = Deno.env.get("OPENAI_API_KEY");
    if (!OPENAI_API_KEY) {
      return new Response(JSON.stringify({ error: "Server misconfigured (OPENAI_API_KEY)" }), { status: 500, headers: { "Content-Type": "application/json", ...corsHeaders } });
    }
    ...
    const instructions = (settings?.instructions && String(settings.instructions)) || Deno.env.get("OPENAI_REALTIME_INSTRUCTIONS")
      || "Tu es l'assistante NotionLab. Réponds en français, de manière concise et chaleureuse. Réponds à l'oral immédiatement quand tu entends l'utilisateur.";
    ...
    if (!upstream.ok) {
      ...
      return new Response(JSON.stringify({ error: "Upstream realtime error", model, voice, details: errText }), { status: 502, headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    const answerSdp = await upstream.text();
    return new Response(JSON.stringify({ sdp: answerSdp }), { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } });
  } catch (e) {
