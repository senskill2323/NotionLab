...
const HOOK_SECRET =
  Deno.env.get("SEND_EMAIL_HOOK_SECRET")?.replace("v1,whsec_", "") ?? "";
const DEFAULT_SENDER_EMAIL =
  Deno.env.get("DEFAULT_EMAIL_SENDER") ??
  Deno.env.get("SMTP_USER") ??
  Deno.env.get("SMTP_USERNAME") ??
  "yann@notionlab.ch";
const DEFAULT_SENDER_NAME =
  Deno.env.get("DEFAULT_EMAIL_SENDER_NAME") ?? "NotionLab";
const TEST_EMAIL_RECIPIENT =
  Deno.env.get("TEST_EMAIL_RECIPIENT") ?? "yann@notionlab.ch";
...
const SMTP_PASSWORD =
  Deno.env.get("SMTP_PASSWORD") ?? Deno.env.get("SMTP_PASS") ?? "";
