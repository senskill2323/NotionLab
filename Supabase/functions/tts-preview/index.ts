...
serve(async (req: Request): Promise<Response> => {
  ...
  const textInput = typeof payload?.text === "string" && payload.text.trim()
    ? payload.text
    : `Bonjour, je suis la voix ${voice}. Ceci est un échantillon pour pré-écoute.`;
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return new Response(JSON.stringify({ error: "OpenAI TTS error", detail: errText }), {
        status: 500,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      });
    }
    ...
    return new Response(JSON.stringify({ error: "Unhandled error", detail: String(e?.message || e) }), {
      status: 500,
      headers: { "Content-Type": "application/json", ...corsHeaders },
    });
  }
