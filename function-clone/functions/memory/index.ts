...
serve(async (req)=>{
  ...
  try {
    ...
    const patch = body && typeof body.patch === "object" && body.patch ? body.patch : undefined;
    ...
    if (action === "update" && !N8N_MEMORY_WRITE_URL) {
      return new Response(JSON.stringify({
        ok: false,
        error: "Server misconfigured (N8N_MEMORY_WEBHOOK_URL)"
      }), {
        status: 200,
        headers: {
          "Content-Type": "application/json",
    ...
    if (action !== "update" && !N8N_MEMORY_SEARCH_URL) {
      return new Response(JSON.stringify({
        ok: false,
        error: "Server misconfigured (N8N_MEMORY_SEARCH_URL)"
      }), {
        status: 200,
        headers: {
          "Content-Type": "application/json",
    ...
    if (action === "update") {
      ...
      if (hasKeyValue) {
        ...
        const upstream = await fetch(withToken(N8N_MEMORY_WRITE_URL, N8N_MEMORY_WRITE_TOKEN), {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
          },
        ...
        if (!upstream.ok) {
          ...
          return new Response(JSON.stringify({
            ok: false,
            error: "Upstream memory webhook error",
            details
          }), {
            status: 200,
            headers: {
      ...
      if (patch && typeof patch === "object") {
        ...
        if (keys.length === 0) {
          return new Response(JSON.stringify({
            ok: false,
            error: "Missing key/value or patch"
          }), {
            status: 200,
            headers: {
              "Content-Type": "application/json",
        ...
        for (const k of keys){
          const v = patch[k];
          const up = await fetch(withToken(N8N_MEMORY_WRITE_URL, N8N_MEMORY_WRITE_TOKEN), {
            method: "POST",
            headers: {
              "Content-Type": "application/json"
            },
    ...
    if (typeof body["query"] === "string") {
      readPayload.query = body["query"];
    }
    if (typeof body["top_k"] === "number") {
      readPayload.top_k = body["top_k"];
    }
    const upstream = await fetch(withToken(N8N_MEMORY_SEARCH_URL, N8N_MEMORY_SEARCH_TOKEN), {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
