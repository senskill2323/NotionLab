...
const deliveryModeConfig = {
  ...
  in_person: { label: "Presentiel", icon: MapPin },
  hybrid: { label: "Hybride", icon: Layers },
};

// Utilitaire pour calculer la duree totale des modules
...
const FormationsSection = ({ content = {} }) => {
  ...
  const subtitle =
    content.subtitle ||
    "Choisissez la formation qui correspond a votre niveau et vos objectifs. Chaque formation est concue pour vous faire progresser rapidement.";
  const backgroundImageUrl =
    content.backgroundImageUrl || "https://images.unsplash.com/photo-1687754946970-5ff99224bd70";
  const ctaConfig = content.cta || {};
  const showCta = ctaConfig.enabled !== false;
  const ctaMode = ctaConfig.backgroundMode === "solid" ? "solid" : "gradient";
  const fallbackGradient = "linear-gradient(90deg, #7c3aed 0%, #4f46e5 100%)";
  const ctaBackground =
    ctaMode === "solid"
      ? ctaConfig.solidColor && ctaConfig.solidColor.trim().length > 0
        ? ctaConfig.solidColor
        : "#4f46e5"
      : ctaConfig.gradient && ctaConfig.gradient.trim().length > 0
      ? ctaConfig.gradient
      : fallbackGradient;
                    ...
                    <div className="w-full h-24 overflow-hidden">
                      <img
                        ...
                        src={
                          ...
                          "https://images.unsplash.com/photo-1526554779127-cef759318656"
                        }
                    ...
                    <CardHeader className="pb-3">
                      <div className="flex items-start justify-between mb-2">
                        <Badge variant="secondary" className="text-xs">
                          {Array.isArray(formation.level)
                            ? formation.level.join(', ')
                            : formation.level}
                        </Badge>
                      ...
                      {modulesList.length > 0 && (
                        <div className="mb-4">
                          <ul className="text-xs text-muted-foreground space-y-1">
                            ...
                            {formation.nodes &&
                              formation.nodes.filter((node) => node.type === "moduleNode").length > 3 && (
                                <li className="text-xs text-muted-foreground/70 italic">
                                  +
                                  {
                                    formation.nodes.filter((node) => node.type === "moduleNode").length -
                                    3
                                  }
                                  {' '}autres modules
                                </li>
